...
class Generator {
    ...
    async getRepo(name = 'zxy2222') {
        // 1）从远程拉取模板数据
        const repoList = await wrapLoading(getRepoList, '获取模板中,别急！！！', name);
        if (!repoList) return;
